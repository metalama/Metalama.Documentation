...
| <xref:aspect-api> | An API that enables you to transform the source code, report diagnostics, and more. |
| <xref:extensions-api> | Useful for extensions based on the `Metalama.Framework` abstractions, including architecture validation, code fixes, and dependency injection. |
...
| <xref:testing-api> | Two testing frameworks: one for unit testing and the second for snapshot-based testing. |
