...
level: 400
summary: "This article explains how to separate the aspect implementation from its custom attribute contract, enabling scenarios like using third-party attributes, preparing for migration away from Metalama, or having different implementations per project."
keywords: "aspect decoupling, custom attribute, IAspect interface, aspect contract, aspect implementation, fabric binding, IRef, IAttribute, Metalama separation"
...
* The custom attribute can reside in a package for which you don't have the source code (for instance, those of the <xref:System.ComponentModel.DataAnnotations> namespace) or where you can't or don't want to add a reference to Metalama.
...
We'll first look at the situation where the project that contains the custom attribute can reference the `Metalama.Framework` project. This approach is simpler because the custom attribute can be constructed into a compile-time object.
...
3. Implement your aspect as usual. The only difference is that the attribute object isn't the aspect object itself but is available in the instance field.
...
In this example, we show a traditional logging aspect whose API is a custom attribute that does not implement the <xref:Metalama.Framework.Aspects.IAspect`1>. However, the contract attribute uses the Metalama-specific <xref:Metalama.Framework.Aspects.RunTimeOrCompileTimeAttribute?text=[RunTimeOrCompileTime]> attribute, which simplifies the implementation of the aspect.
...
If you can't or don't want to add a reference to `Metalama.Framework` in the contract project, the process is more difficult. The reason is that the custom attribute is a _run-time only_ class, which means that it _can't_ be constructed at build time. However, the aspect constructor and its `BuildAspect` method all run at _build time_. This means that you can't pass the attribute itself to the aspect.
...
> <xref:Metalama.Framework.Code.IAttribute>
> <xref:Metalama.Framework.Code.IRef`1>
> <xref:Metalama.Framework.Serialization.ICompileTimeSerializable>
