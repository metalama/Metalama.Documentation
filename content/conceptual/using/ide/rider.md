...
JetBrains Rider supports Metalama's core functionality out of the box. If you're creating aspects and want to test them, some additional configuration is required.
...
Metalama's aspect testing framework uses a custom xUnit test discovery mechanism. By default, Rider uses metadata-based discovery and won't detect aspect tests. Configure Rider to use the test runner for discovery.
...
The following Metalama features are not available in Rider:
...
1. Verify `Metalama.Testing.AspectTesting` is referenced (this disables Metalama processing)
2. Check that `.t.cs` files are not being compiled (the package should exclude them automatically)
