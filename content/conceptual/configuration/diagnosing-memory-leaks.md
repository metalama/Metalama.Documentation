...
> [!NOTE]
> The diagnostic runs during a **normal command-line build**, not in the IDE. Nothing is leaking during that build, because the compiler exits when it finishes. What the build reports is the *shape* of what your code left behind, and that shape is the same in both cases, so a command-line build tells you what an editing session would retain. Running the analysis inside the IDE instead would add its cost to the very process it's meant to protect.
...
* Retentions in **Metalama itself**, which are only counted. Report these to us through [GitHub](https://github.com/metalama/Metalama/issues) if you wish, but they aren't something you can act on, and a non-zero count is normal.
...
* A **durable** reference, of type <xref:Metalama.Framework.Code.IDurableRef> or <xref:Metalama.Framework.Code.IDurableRef`1>, stores only a string identifier and holds nothing else. It's safe in an object of any lifetime, and it's what Metalama itself stores in its own long-lived objects.
* Any **other** reference, such as one returned by <xref:Metalama.Framework.Code.IDeclaration.ToRef*>, holds the symbol and the project snapshot behind it. It's correct and fast within a run, and it retains a snapshot as soon as you store it in something that outlives the run.
...
The same applies to a plain <xref:Metalama.Framework.Code.IRef`1> obtained from <xref:Metalama.Framework.Code.IDeclaration.ToRef*>. A reference resolves in any project snapshot, which makes it the right way to *pass* a declaration between aspects, but it holds the symbol and the snapshot behind it, so it isn't the right way to *store* one.

### Store a durable reference instead

Call `ToDurableRef()` on the declaration, or `ToDurable()` on a reference you already have, and resolve the result later with `GetTarget( compilation )` exactly as you would resolve any other reference. A durable reference holds only a string identifier. Resolving one costs an identifier lookup, which is why references aren't durable by default.
...
[!code-csharp[](~/code/Metalama.Documentation.SampleCode.AspectFramework/RetainedDeclaration_Wrong.Fabric.cs?range=3-999)]

Changing the type of the field is the whole fix:

[!code-csharp[](~/code/Metalama.Documentation.SampleCode.AspectFramework/RetainedDeclaration_Good.Fabric.cs?range=3-999)]

### Generic types and cross-project references

* A durable reference is backed by a *declaration* identifier, so a constructed generic type such as `Base<int>` resolves back to `Base<T>` and its type arguments are lost silently. Where the value is a type whose shape you don't control, store its full name instead and match on that.
